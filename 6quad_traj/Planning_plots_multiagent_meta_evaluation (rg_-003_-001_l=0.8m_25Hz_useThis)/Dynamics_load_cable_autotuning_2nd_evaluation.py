...
class multilift_model:
    ...
    def payload_dyn(self, xl, ul):
        ...
        # dvl     = -self.g*self.ez + 1/self.ml * Fl
        # dwl     = inv(self.Jl)@(Mtl-S_wl@(self.Jl@wl))
        # Model
    ...
    def minisnap_load_circle(self,Coeffx,Coeffy,Coeffz,time):
        ...
        alpha  = 2*np.pi/self.nq
        beta   = 4/9*np.pi # not needed to be collision-free
        ref_xq = np.zeros(int(self.nq)*self.nxi) # in the world frame
        ref_uq = np.zeros(int(self.nq)*self.nui)
        for i in range(int(self.nq)):
            ...
            ref_wi   = np.zeros((3,1))
            # ref_dwi  = np.zeros((3,1))
            # ref_ddwi = np.zeros((3,1))
            ref_ti   = np.array([[self.ml*self.g/self.nq]])
            ref_dti  = np.zeros((1,1))
            ref_xi   = np.reshape(np.vstack((ref_di,ref_wi,ref_ti,ref_dti)),self.nxi)
            # ref_xi   = np.reshape(np.vstack((ref_di,ref_ti)),self.nxi)
